...
local function next_action()
  ...
  frames_to_hold = (self.production.UNIT_COSTS[SCRIPTED_ACTIONS[action_index]] - self.production.UNIT_COSTS['fighter']) / self.production.BUILDING_SPEED + 2 + math.random() * 60
  
  -- how much time to wait before actually performing the scripted action?
  if not self.resources then return end
  local missing_resources = (self.production.UNIT_COSTS[SCRIPTED_ACTIONS[action_index]] - self.resources.amount) / self.resources.harvest_rate
